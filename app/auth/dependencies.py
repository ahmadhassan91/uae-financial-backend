...
async def get_current_admin_or_ops_user(
    current_user: User = Depends(get_current_user)
) -> User:
    """
    Get current user and verify admin or ops privileges.
    
    This dependency allows:
    - full admin users (all access)
    - ops admin users (Company Management access)
    
    It blocks view_only users from accessing these endpoints.
    Use this for Company Management module routes.
    """
    if not current_user.is_admin:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
    ...
    # Allow full and ops roles, block view_only
    if hasattr(current_user, 'admin_role') and current_user.admin_role == "view_only":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="View-only access: This action requires full admin or ops privileges"
        )
    return current_user
